...
import { CATEGORIES, LINKS, TIMER_VALUES } from '../../../constants/index';
...
const Navbar = ({
            ...
            <div key={category} className="nav__dropdown-item">
                ...
                <ul className="widget">
                    {_.map(widgets, widget => (
                        <li key={widget} className="widget__item">{widget}</li>
                    ))}
                </ul>
            </div>
        ))
    ...
    const renderLink = () => {
        const setActiveClass = (name, path) => {
            ...
            } else if (name.toLowerCase() === 'shop' && ['product', 'product-category'].includes(pathname.split('/')[1])) {
                return ' nav__link--active';
        ...
        return (
            _.map(LINKS, ({ name, path }) => (
                <li onClick={handleHideModal} key={path} className="nav__item">
                    <Link to={path} className={`nav__link${setActiveClass(name, path)}`}>{name}</Link>
                </li>
            ))
        )
    };
        ...
        <Fragment>
                    ...
                    <div className="row u-padding-top-small u-padding-bottom-small">
                        <div className="col span_1_of_8">
                            <div className="action-bar__logo">
                                <a href="/">
                                    <img className="action-bar__img"
                                        src="https://res.cloudinary.com/shopping-assets/image/upload/v1584263975/logo_lrcmli.png"
                                        alt="Logo" />
                                </a>
                            </div>
                        </div>
                        {window.innerWidth > 900 && <div ref={searchWrapper} className="col span_5_of_8" style={{ position: 'relative' }}>
                            <form className="form" method="dialog">
                                <input
                                    ...
                                    ref={inputRef}
                                />
                                {isLoading && <img className="loading__image-small" src={Loading} alt="" />}
                            ...
                            <div className="row-fluid">
                                <div className="col span_1_of_2">
                                    <div className="action-bar__box-content">
                                        <i onClick={() => setExpandMenu(!expandMenu)} className={`${expandMenu ? 'far fa-times-circle u-suggest' : 'fas fa-bars u-d-none u-suggest'}`} />
                                        <Link to="/wishlist">
                                        ...
                                        <div
                                            ...
                                            <div className="action-bar__content-right">
                                                <span className="action-bar__text--sub">{`${displayName ? 'Hello, Sign out' : 'Hello, Sign in'}`}</span>
                                                <span className="action-bar__text--main">{`${displayName ? _.capitalize(displayName) : 'My account'}`}</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div
                                        ...
                                        <div className="action-bar__content">
                                            <div className="action-bar__content-left">
                                                ...
                                                {state.length ? < span className="action-bar__cart">{countQuantity()}</span> : null}
                                            </div>
                                            <div className="action-bar__content-right">
                                                <span className="action-bar__text--sub">My cart</span>
                                                <span className="action-bar__text--main">{formatter.format(countTotalPrice(state))}</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
            ...
            <div className="navbar navbar--mobile" id="navbarMobile">
                <div className="row">
                    ...
                    <div className="col span_1_of_3">
                        <div className="box-icons">
                            <i ref={demoRef} onClick={() => demo()} className={`${isOpenSearchMobile ? 'fas fa-search-minus' : 'fas fa-search-plus'}`}></i>
                            <Link to="/cart">
                                <div className="action-bar__content-left">
                                    <i className="fas fa-cart-plus" />
                                    {state.length ? <span className="action-bar__cart">{countQuantity()}</span> : null}
                ...
                <div className="row u-padding">
                    {(window.innerWidth <= 900 && isOpenSearchMobile) && (<div
                        ref={searchWrapper}
