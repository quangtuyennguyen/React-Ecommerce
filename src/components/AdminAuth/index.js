...
function AdminAuth({ adminAuth, toggleLoading }) {
    ...
    const validate = ({ email: emailCurrent, password: passwordCurrent }) => {
        ...
        } else if (!/^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,4}$/i.test(emailCurrent)) {
    ...
    return (
        ...
        >
            {({ handleSubmit, errors, touched }) => (
                <Form className="form form-admin-auth" onSubmit={handleSubmit} autoComplete="off">
                    ...
                    {touched.password && <span className="form__text-error">{errors.password}</span>}
                    <div>
