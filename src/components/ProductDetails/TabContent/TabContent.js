...
import CommentList from './TabComments/CommentList/CommentList';
import { TabDescription } from './TabDescription/Description';
import { TabInfo } from './TabInfo/TabInfo';

const TabContent = ({
    productComments: comments, sortComments
}) => {
        ...
        _.map(tabs, ({ tab, component }, index) => (
            <li key={tab} className="tab__item">
                <span
                    ...
                    className={classnames('tab__link', { 'tab__link--active': index === indexTabCurrent })}>
                    {tab}
                </span>
            </li>
        ))
        ...
        <section id="section-wrapper-tabs">
            <div className="row u-border">
                <div className="tabs__heading  u-underline u-margin-bottom-medium">
                    <ul className="tabs__list u-d-flex-left">
                        {renderTabs()}
                    </ul>
                </div>
