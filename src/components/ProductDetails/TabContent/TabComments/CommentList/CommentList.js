...
import { renderRating, sortComments, filterProductComments, countTotalRating } from '../../../../../utils';
import { SortComment } from '../../SortComment/SortComment';
import CommentItem from '../CommentItem/CommentItem';
import { CommentStat } from '../CommentStats/CommentStat';
...
const CommentList = ({
    ...
    return (
        <div className="tab__reviews moveToTop">
            {filterComments.length ? <div className="wrapper-stats u-border-small u-padding-bottom-medium">
                <div className="row">
                    <div className="col span_1_of_3">
                        <div className="comment__stats">
                            <h3 className="comment__title">{filterComments.length} Reviews</h3>
                            <div className="u-d-flex-left">
                                <div className="product__rating">
                                    {renderRating(4)}
                                </div>
                                <span className="comment__text">{Math.round(countTotalRating(filterComments) / filterComments.length * 10) / 10} Overall rating</span>
                            </div>
                            <p className="comment__desc">58 out of 74 (77%)
                        <br />
                    ...
                    <div className="col span_2_of_3">
                        <div className="progress">
                            <CommentStat comments={filterComments} totalComment={filterComments.length} />
                        </div>
                    </div>
                </div>
            </div> : null}
                ...
                <div className="row">
                    <div className="col span_3_of_5">
                        <div className="comment-box">
                            {renderCommentItem()}
                            {(filterComments.length > 3) ? (commentsCurrent < filterComments.length ?
                                <button onClick={() => handleShowComment(3)} className="btn btn--outline">Load more reviews</button> :
                                <button onClick={() => handleShowComment(-(commentsCurrent - 3))} className="btn btn--outline">Load less reviews</button>
                            ) : null}
                        </div>
                    </div>
