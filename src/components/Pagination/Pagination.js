    ...
    const renderPagination = () => {
        if (window.innerWidth >= 600) {
            ...
            for (let i = 0; i < total; i++) {
                pagination.push(
                    <li key={i} className="pagination__item">
                        <button
                            disabled={(currentPage === 0 && i === 0) || (currentPage === total - 1 && i === total - 1)}
                            onClick={() => setCurrentPage(i)}
                            className={classnames('pagination__btn', { 'pagination__btn--active': i === currentPage })}>
                            {i + 1}
                        </button>
                    </li>
                )
            ...
            return <div className="pagination__list">
                ...
                <span style={{paddingLeft: '0.4rem'}} className="pagination__total">{total}</span>
            </div>
        }
    };
