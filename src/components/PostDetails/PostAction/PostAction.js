...
export const PostAction = ({ postNears }) => {
        ...
        <div className="row row--control u-margin-top-medium u-margin-horizontal-none">
                ...
                {(postReview === 'prev' && prevPost) && (
                    <div className="post__preview-box post__preview-box--prev">
                        <div className="post_preview-image-box">
                            <img className="post__preview-image" src={prevPost.useravatar} alt="" />
                        </div>
                        <div className="post__preview-text">
                            <h6 className="post__preview-title">{_.capitalize(prevPost.title)}</h6>
                            <span className="post__preview-desc">{prevPost.authorName}</span>
                        </div>
                    </div>
                )}
                <Link to={prevPost ? `/blog/${slug(prevPost.title.toLowerCase())}.${prevPost.id}` : pathname}>
                    <div
            ...
            <div className="col span_1_of_3">
                {(postReview === 'next' && nextPost) && (
                    <div
                        className="post__preview-box post__preview-box--prev">
                        <div className="post_preview-image-box">
                            <img className="post__preview-image" src={nextPost.useravatar} alt="" />
                        </div>
                        <div className="post__preview-text">
                            <h6 className="post__preview-title">{_.capitalize(nextPost.title)}</h6>
                            <span className="post__preview-desc">{nextPost.authorName}</span>
                        </div>
                    </div>
                )}
                <Link to={nextPost ? `/blog/${slug(nextPost.title.toLowerCase())}.${nextPost.id}` : pathname}>
                    <div
