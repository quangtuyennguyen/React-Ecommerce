...
const SideBar = ({
    products, rangeValue,
    filterByRange, filterBySize, range,
    setRange, filterByRating, sizeValue,
    category: categoryProduct, ratingValue
}) => {
    const [categoryMore, setCategory] = useState(true);
    const [sizeMore, setSize] = useState(true);
    const totalProduct = products.length;

    const renderCategoryQuantity = value => _.filter(products, ({ category }) => category === value).length
    const renderSizeQuantity = size => _.filter(products, ({ sizes }) => sizes && sizes.indexOf(size) !== -1).length
            ...
            if (index > 4 && categoryMore) {
                ...
                return (
                    <li key={category} className="shopsidebar__item">
                        <Link
                            className={classnames('shopsidebar__link', { 'shopsidebar__link--active': categoryProduct === _.kebabCase(category) })}
                            to={`/product-category/${_.kebabCase(category)}`}>
                            {category}
                            <span className="shopsidebar__quantity">{renderCategoryQuantity(category)}</span>
                        </Link>
                    </li>
                );
            };
    ...
    const renderRatings = () => (
        _.map([5, 4, 3, 2, 1], item => (
            <div key={item} className="shopsidebar__rating-item">
                <input readOnly checked={ratingValue === item ? true: false} className="shopsidebar__radio" type="radio" name="rating" id={`_${item}rating`} />
                <label
                    ...
                    <div className="shopsidebar__rating">
                        {_.map([0, 1, 2, 3, 4], star => (
                            <i key={star} className={`${star >= item ? 'far' : 'fas'} fa-star`}></i>
                        ))}
                    </div>
                    <span
                        className="shopsidebar__rating-text">
                        {`${item < 5 ? 'bigger' : ''} ${item} ${item > 1 ? 'stars' : 'star'}`}</span>
                </label>
            </div>
        ))
    );
    ...
    if (search) {
        sizes = _.filter(sizes, size => size.toLowerCase().indexOf(search.toLowerCase().trim()) !== -1);
    };
                ...
                return (
                    <li key={size} className="filter__item u-d-flex-center">
                        <div className="filter__control">
                            <input readOnly className="checkbox" checked={sizeValue.includes(size) ? true : false} type="checkbox" name="filter" id={size} />
                            <label
        ...
        <div className="shopsidebar shopsidebar--filter">
            <div className="shopsidebar__category">
                ...
                <ul className="shopsidebar__list">
                    <Link to="/shop" className="shopsidebar__link">All
            ...
            <div className="shopsidebar__price" style={{ border: 0 }}>
                <h5 className="shopsidebar__heading" style={{ paddingBottom: '3.6rem' }}>Price</h5>
                <RangeSlider
                    range={range}
                    setRange={setRange}
