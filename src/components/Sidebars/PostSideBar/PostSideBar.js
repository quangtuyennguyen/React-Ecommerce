...
import { BLOG_TAGS } from '../../../constants/index';


export const PostSideBar = ({ otherPosts }) => {
    const { pathname } = useLocation();

    const renderTags = () => (
        _.map(BLOG_TAGS, tag => (
            <Link key={tag} to={pathname} className="btn btn--outline--small">{tag}</Link>
        ))
    );
    ...
    return (
        <div className="col span_1_of_3 u-padding-left-regular">
            <div className="post__trending">
                ...
                <ul className="post__trending-list">
                    {renderPosts()}
                </ul>
            </div>
            ...
            <div className="post__banner u-center-text u-margin-top-medium">
                <img src="https://demo.createx.studio/cartzilla/img/blog/banner-bg.jpg" alt="" className="post__image" />
