...
const ProductDetailsContainer = ({ products }) => {
    ...
    const firstLetter = title.split('').shift().toUpperCase();

    useEffect(() => {
        document.title = `${firstLetter}${title.replace(title[0], '')} - Product`;
    }, [firstLetter, title]);

    const findProductCurrent = () => _.find(products, product => product.id === id);
    const productCurrent = findProductCurrent();
    const { category: categoryCurrent } = productCurrent || {};
    const otherProducts = [..._.filter(products, ({ category }) => category === categoryCurrent)];
    return (
        productCurrent ? (
            <Fragment>
