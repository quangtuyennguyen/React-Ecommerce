import { postCommentsRef, postsRef, productCommentsRef, productsRef } from '../services/firebase';
import { fetchDataRequest, findKeyData, pushItem, removeItem, updateItem } from '../utils';
...
export const fetchProducts = callback => async dispatch => {
    try {
        ...
        dispatch({ type: types.FETCH_PRODUCTS_FAILD, payload: 'Fetch product faild' });
    };
};
...
export const fetchProductCommments = callback => async dispatch => {
    try {
        ...
        dispatch({ type: types.FETCH_PRODUCT_COMMENT_FAILD, payload: 'Fetch product faild' });
    };
};
...
export const fetchPostComments = callback => async dispatch => {
    try {
        ...
        dispatch({ type: types.FETCH_POST_COMMENTS_FAILD, payload: 'Fetch comment posts faild' });
    };
};
