...
export const resultIndex = (arr, value) => _.findIndex(arr, ({ id }) => id === value);
export const ranDomAvatar = size => `https://randomuser.me/api/portraits/${Math.random() > 0.5 ? 'women/' : 'men/'}${Math.round(Math.random() * size)}.jpg`;
export const pricePerProduct = ({ price, quantity }) => formatter.format(price * 1.01 * quantity);
...
export const formatProductFields = values => {
    ...
    if (typeof sizes === 'string' && sizes) {
        result = { ...result, sizes: strToArr(sizes).map(item => item.toUpperCase()) };
    };
    if (sale) {
        result = { ...result, sale };
    };
    ...
    return { ...resultFinal, price: parseInt(price), rating: parseInt(rating), discountEndTime: parseInt(discountEndTime) };
};
...
export const filterProductComments = (comments, id) => (
    _.filter(comments, ({ productId }) => (
        productId === id
    ))
);

export const countTotalComments = comments => _.reduce(comments, (a, { replys }) => a + (replys ? Object.keys(replys).length : 0), 0);
...
export const filterByCategory = (products, value) => (
    _.filter(products, ({ category }) => _.lowerCase(category) === _.lowerCase(value))
);
...
export const sortComments = (comments, { sortBy, sortValue }) => (
    comments.sort((a, b) => {
        if (sortBy === 'time') {
            ...
            if (a.rating < b.rating) { return -sortValue };
            if (a.rating > b.rating) { return sortValue };
        }
        return 0;
    })
...
export const renderRating = rating => (
    _.map([1, 2, 3, 4, 5], number => <i key={number} className={`${number > rating ? 'far' : 'fas'} fa-star`} />)
);

export const filterAndCountComments = (comments, id) => {
    const newComments = _.filter(comments, ({ postId }) => (
        postId === id
    ));
    const totalReplys = _.reduce(newComments, (a, { replys }) => a + (replys ? Object.keys(replys).length : 0), 0);
...
export const formatField = str => str.split(',').map(elm => elm.trim()).filter(elm => elm !== '');
